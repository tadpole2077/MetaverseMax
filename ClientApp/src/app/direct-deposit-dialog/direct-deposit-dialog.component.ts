...
export class DirectDepositDialogComponent {  
    ...
    ethereum: any;
    accountMCPMegaBalance = 0;
    accountMCPMegaAllowance = 0;
    currentPlayerWalletKey = '';
    subscriptionAccountActive$: Subscription;
    subscriptionBalanceChange$: Subscription;
    amountDepositControl = new FormControl('0');

    depositRotateActive = false;
    networkCheckActive = false;
    networkChange = false;
    networkMsg: string;
    networkWarning = false;

    contractCheckMsg = 'Confirming MCP Contract Valid';
    checkingContract = true;
    ...
    depositFocus = true;
    withdrawFocus = false;
    insufficientMsg = 'Insufficient Allowance';
                  ...
                  this.getMCPMegaBalance().then((megaBalance) => {
                      ...
                      this.getMCPMegaAllowance().then((MCPMegaAllowance) => {
                          ...
                          this.zone.run(() => {
                              ...
                              const orgValue = this.amountDepositControl.value;
  ...
  async getMCPMegaAllowance() {
      ...
      const owner = this.currentPlayerWalletKey;
      const spender = this.globals.selectedWorld == WORLD.BNB ? this.CONTRACT_MCPMEGA_MANAGER_BNB : this.CONTRACT_MCPMEGA_MANAGER_ETH;   // spender  MCP MEGA Manager contract - handles deposits into Mega world
  ...
  async depositMegaToMCP() {
      ...
      //  return;
      //}
      const selectedNetworkCorrect: boolean = await this.manageNetwork();
  ...
  convertToCoinNumber(amount: any, allowanceExtra = 0): string{
      ...
      let decimalLength = 0;
      const megaDecimalCount = this.MEGA_DECIMALS_COUNT;
  ...
  checkMCPContractValid() {
      const contractName = this.globals.selectedWorld == WORLD.BNB ? MCP_CONTRACT_NAME.DEPOSIT_BSC : MCP_CONTRACT_NAME.DEPOSIT_ETHEREUM;
      const contractAddressCompare = this.globals.selectedWorld == WORLD.BNB ? this.CONTRACT_MCPMEGA_DEPOSIT_BNB : this.CONTRACT_MCPMEGA_DEPOSIT_ETH;
