...
export class TESTBankManageComponent {
    ...
    async withdrawAllowanceApprove(amount: number, ownerMaticKey: string) {
        ...
        let params = new HttpParams();
        const signResult = await this.walletSign(amount);
    ...
    convertToCoinNumber(amount: number, allowanceExtra = 0): string{
    ...
    async startWebProcess() {
        ...
        await this.initWeb3();
        const connected = await this.checkNetwork(HEX_NETWORK.BINANCE_TESTNET_ID, 'Binance Testnet');
    ...
    async contractTestSendRequest(addressTo: string, addressFrom: string) {
        ...
        const token_uint256 = this.utils.padLeft(this.utils.toHex(valuePara1), 64).substring(2);
        //let transferData =
        //  contractMethod +
        //  token_uint256;
      ...
      this.rotateActive = false;
        ...
        try {
            const from_address = this.utils.padLeft(CONTRACT_ADDRESS, 64).substring(2);
...
*/
    ...
    async contractTestSeedAccount(addressFrom: string) {
        ...
        try {
            ...
            const from_address = this.utils.padLeft(addressFrom, 64).substring(2);
            const token_uint256 = this.utils.padLeft(this.utils.toHex(valuePara1), 64).substring(2);
