...
import { IOwnerLandData, IOwnerData, IFilterCount, ICoordinates } from './owner-interface';
...
export class OwnerDataComponent implements AfterViewInit, OnDestroy {
            ...
            if (requestOwnerMatic.toLowerCase() == 'myportfolio') {
                ...
                if (this.globals.ownerAccount.checked == false && this.checkInstance < 4) {
                    ...
                    if (this.notifySubscription == null) {
                        ...
                        this.notifySubscription = interval(1000).subscribe( () => {
    ...
    searchOwnerbyMatic(ownerMatic: string, forceCDFrefresh: boolean = false) {
        ...
        this.httpOwnerDataService.searchOwnerbyMatic(ownerMatic)
            .subscribe({
                next: (result) => {
    ...
    searchPlot(plotPos: ICoordinates, loadBuildingHistory:boolean = false) {
        ...
        this.httpOwnerDataService.searchPlot(plotPos).subscribe({
            next: (result) => {
