...
export class BalanceManageDialogComponent implements OnInit, OnDestroy{
  ...
  async checkBalances(MCPMegaCheck) {
      ...
      this.zone.run(() => {
          ...
          const orgValue = this.amountWithdrawControl.value;
  ...
  async depositMegaToMMBankWithAllowance() {
      ...
      const testnet:boolean = await this.checkBNBTestnet();
  ...
  async getMMBankMegaAllowance() {
      ...
      const spender = this.globals.selectedWorld == WORLD.BNB ? this.CONTRACT_MMBank : this.CONTRACT_MMBank;   // spender  MM Bank contract - handles deposits into MMBalance
  ...
  async withdrawAllowanceApprove() {
      ...
      const signResult = await this.walletSign(withdrawMegaAmountNumber);
  ...
  async checkBNBSmartChain() {
      ...
      let chainIdHex: string;
      let bnbSmartChain = false;

      const connected = await this.checkNetwork(HEX_NETWORK.BINANCE_ID, 'Binance Mainnet');
